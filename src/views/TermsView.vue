<template>
  <div class="container">
    <div class="wrapper d-flex flex-column align-items-center my-3">
      <h1 class="text-center my-3">服務條款</h1>
      <div class="terms py-3 px-4 mb-5 mx-3">
        <p>
          「BGIP」是由「BGIP」（下稱我們）所經營之網站(下稱本網站)各項服務與資訊。
          ...
          （3）本網站將蒐集的數據用於各種目的： •提供和維護系統所提供讀服務
          •提供用戶支持 •提供分析或有價值訊息，以便我們改進服務
          •監控服務的使用情況 •檢測，預防和解決技術問題
          ...
          （5）您同意於使用本網站服務時，所提供及使用之資料皆為合法，並無侵害第三人權利、違反第三方協議或涉及任何違法行為。如因使用本APP服務而致第三人損害，除因我們故意或重大過失所致外，我們並不負擔相關賠償責任。
          四、對外的相關連結
          ...
          （2）前項但書之情形包括但不限於： •經由您書面同意。 •法律明文規定。
          •為維護國家安全或增進公共利益。
          •為免除您生命、身體、自由或財產上之危險。
          ...
          。 以下是可能使用的Cookie範例: •session cookies.
          用來維護應用程式的狀態 •Preference Cookies. 用來記錄您的喜好與設定
          •Security Cookies. 用來控制安全和檢查 七、未成年人保護
    ...
    <div class="text-center my-3">
      <button @click="this.$router.go(-1)" class="mainBtn">回上一頁</button>
    </div>
  </div>
</template>
