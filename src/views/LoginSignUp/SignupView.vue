    ...
    async handleSubmit(userInfo) {
      if (userInfo.email.length > 20) {
        ...
        createUserWithEmailAndPassword(auth, userInfo.email, userInfo.password)
          .then((userCredential) => {
            // this.$store.commit("setUser", userCredential.user.uid);
            var date = new Date();
            var dateresult = date.toDateString();
            this.userInfo.uid = userCredential.user.uid;
            setDoc(doc(db, "Account", this.userInfo.uid), {
              PhotoURL:
